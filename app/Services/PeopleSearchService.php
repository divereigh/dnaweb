    ...
    private function baseQuery(string $q, int $linked, int $hasMatches, bool $count): array
    {
        ...
        if ($q !== '') {
            ...
            if ($lex === '' && $phon === '') {
                ...
                // MariaDB optimiser bug: OR'd MATCH expressions combined
                // with a filesort-requiring ORDER BY silently drop rows
                // (the row-by-row MATCH path returns 0 score when
